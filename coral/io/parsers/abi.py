'''Module for parsing ABI trace files.
...
class ABI(object):
    ...
    def __init__(self, in_file, trimming=False):
        ...
        try:
            ...
            # file type, file, version, tag name, tag number, element type
            # code, element size, number of elements, data size, data offset,
            # handle, file type, file version dictionary for containing file
            # metadata
            self.data = {}
            ...
            self._handle.seek(0)
            read = self._handle.read(struct.calcsize(_HEADFMT))
            header = struct.unpack(_HEADFMT, read)
            # file format version
